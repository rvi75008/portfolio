...
def wait_for_container(
    checker_callable, host_port, image, skip_exception=None, timeout=60
):
    ...
    for i in range(timeout):
        ...
        pytest.fail(f"Cannot start {image} server")  # pragma: no cover
...
def container_starter(request, docker, docker_pull):
    def f(
        ...
        if volumes is not None:
            volumes = [vol.split(":")[1] for vol in volumes]  # pragma: no cover

        container_name = "-".join([slugify(image), "server"])
...
def service_container(unused_port, container_starter):
    def f(service_name, checker_callable=None, skip_exception=None, timeout=60):
        with open(f"{path.dirname(__file__)}/docker-compose.yml") as docker_comppse_yml:
            docker_conf = yaml.load(docker_comppse_yml, Loader=yaml.FullLoader)
        service_conf = docker_conf[service_name]
        volumes = service_conf.get("volumes")
        if volumes is not None:
            volumes = [
                path.join(path.dirname(__file__), vol) for vol in volumes
            ]  # pragma: no cover
        params = {
