    ...
    async def extract_value(
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url) as resp:
                ...
                return (
                    float(soup.contents[scrapping_config["content_index"]].split()[-1])
...
def prepare_criteria(criteria: List[str], to_scrap: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    prepared_criteria = []
...
def run_scrapping() -> None:
    ...
    loader.update_from_list_of_dicts(
        ...
        col_to_update=scrappings_config["col_to_update"],
        criteria=prepare_criteria(scrappings_config['criteria'],scrappings_config['to_scrap'])
    )
